...
const Personal = () => {
  ...
  const permisos = ["Ventas", "Inventario", "Clientes", "Pedidos", "Perfil"];
  ...
  const guardarEmpleado = (evento) => {
    ...
    if (ModalEdicion && personaSelect) {
      setPersonas(
        personas.map((p) => (p.id === personaSelect.id ? { ...formulario } : p))
      );
    } else {
  ...
  return (
    <main>
      <div className="titulo">
        <h1>Personal</h1>
        <BotonAgregar onClick={abrirModalAgregar}></BotonAgregar>
      </div>
